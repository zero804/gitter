...
module.exports = function(uri) {
  ...
  if (reservedNamespaceHash[parts[0]]) { return false; }
  if (parts.length > 1 && reservedSubNamespaceHash[parts[1]]) { return false; }

  if (/\/archives\/(all|\d{4}\/\d{2}\/\d{2})/.test(uri)) { return false; }
  if (/^\/orgs\//.test(uri)) { return false; }
