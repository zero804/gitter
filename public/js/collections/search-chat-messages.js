...
module.exports = BaseResolverCollection.extend({
  ...
  initialize: function(models, attrs) {
    ...
    if (!attrs || !attrs.roomMenuModel) {
      throw new Error(
        'A valid instance of RoomMenuModel must be passed to a new instance of SearchChatMessages'
      );
    }
    ...
    if (!attrs || !attrs.roomModel) {
      throw new Error(
        'A valid instance of a roomModel must be passed to a new Instance of SearchChatMessages'
      );
    }
    ...
    this.contextModel = new ContextModel(null, {
    ...
    this.listenTo(
      this.queryModel,
      'change:isFetchingMoreSearchMessageResults',
      this.onModelChangeFetchStatus,
      this
    );
  ...
  onModelUpdateActive: function(model, val) {
    //jshint unused: true
    return !val ? this.reset() : this.fetchResults();
  },

  onSearchTermUpdate: function(model, val) {
    //jshint unused: true
    this.resetSearch();
    if (!this.contextModel.get('active')) {
      return;
    }
    if (!val) {
      return this.reset();
  ...
  fetchResults: _.debounce(function() {
    if (this.queryModel.get('hasReachedLimit')) {
      return;
    }
