...
(function (global, Date, ArSlice, undef) {
    ...
    var ISOIntervalRegExp = /^([\+\-])?P((?:\d+(?:[,\.]\d+)?[YMWD])+)?(?:T((?:\d+(?:[,\.]\d+)?[HMS])+))?$/
    ,   isoGetSet = {
            Y: 'FullYear',
            M: 'Month',
            D: 'Date',
    ...
    function parseISOInterveralFragment(matches, neg, isTime) {
        ...
        ,   n
        ,   dateprops = ['Y', 'M', 'D', 'TH', 'TM', 'TS', 'TMS']
        ,   datevals = [12, 30, 24, 60, 60, 1000, 1000];
        ...
        while(i--) {
            ...
            matches[i] = matches[i].match(/^(\d+)([,\.]\d+)?(\w)/);
            ...
            if (matches[i][3] === 'W') {
                this[neg + isoGetSet.D](+maches[i][0] * 7);
            } else {
                
                // Set the property using "subValue() or addValue()"
                this[ neg + isoGetSet[matches[i][3]] ].call(this, +matches[i][1]);
                
                // If we have a decimal, look for the next property in the line
                if ((+matches[i][2] || 0) > 0) {
                    // Parse the split decimal also:
                    n = AindexOf(dateprops, +matches[i][3]) + 1 || dateprops.length;
                    
                    this[ neg + isoGetSet[ dateprops[n] ] ](+matches[i][1] * datevals[n]);
                }
            }
        }
    };
    ...
    Tempus.addParser(
        ...
        function (a) { return ISOIntervalRegExp.test(a) },
        // Our parser function
        function (s, d) {
            var matches;
            ...
            if (!(s = s.match(ISOIntervalRegExp))) return false;
            
            // Match the date portion:
            if (s[2] && (matches = s[2].match(/\d+(?:[,\.]\d+)?[YMWD]/g))) {
                parseISOInterveralFragment.call(this, matches, s[1] == '-');
            }
            // Match the time portion:
            if (s[3] && (matches = s[3].match(/\d+(?:[,\.]\d+)?[HMS]/g))) {
                parseISOInterveralFragment.call(this, matches, s[1] == '-', true);
            }
            return this;
        },
        // We expect 1st arg to be string
