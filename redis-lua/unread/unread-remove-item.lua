...
local troupeId = table.remove(ARGV, 1)
local itemId = table.remove(ARGV, 1)

local key_count = #KEYS/2

local updated_badge_count_positions = {}

for i in 1,key_count do
	...
	if redis.call("SREM", user_troupe_key, itemId) > 0 then
		...
		if tonumber(redis.call("ZINCRBY", user_badge_key, -1, troupeId)) <= 0 then
