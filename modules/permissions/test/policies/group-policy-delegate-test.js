...
describe('group-policy-delegate', function() {
  ...
  var FIXTURES = [
    { name: 'is group admin', groupId: GROUP_ID, user: USER, isGroupAdmin: true, policy: 'GROUP_ADMIN', expectedResult: true },
    { name: 'is not group admin', groupId: GROUP_ID, user: USER, isGroupAdmin: false, policy: 'GROUP_ADMIN', expectedResult: false },
    { name: 'anonymous', groupId: GROUP_ID, user: null, isGroupAdmin: false, policy: 'GROUP_ADMIN', expectedResult: false },
    { name: 'invalid policy, non-anon', groupId: GROUP_ID, user: USER, isGroupAdmin: true, policy: 'INVALID', expectedResult: false },
    { name: 'invalid policy, anon', groupId: GROUP_ID, user: null, isGroupAdmin: false, policy: 'INVALID', expectedResult: false },
  ];
  ...
  FIXTURES.forEach(function(meta) {
    it(meta.name, function() {
      ...
      var GroupPolicyDelegate = proxyquireNoCallThru('../../lib/policies/group-policy-delegate', {
        '../policy-factory': {
          createPolicyForGroupIdWithUserLoader: Promise.method(function(pUserId, pUserLoader, pGroupId) {
