...
function userSocketConnected(userId, socketId, connectionType, clientType, troupeId, eyeballState, callback) {
  if(!socketId) return Promise.reject(new StatusError(400, 'socketId expected')).nodeify(callback);
  ...
  if(!userId) {
    return redisClient.presenceAssociateAnon(
      ...
      /* values */ socketId, Date.now(), isMobileConnection ? 1 : 0, clientType, troupeId || null)
  ...
  return redisClient.presenceAssociate(
    ...
    /* values */ userId, socketId, Date.now(), isMobileConnection ? 1 : 0, clientType, troupeId || null)
