...
function executeBatch(urlBase) {
  ...
  var operations = [
    ...
    /* Load rooms */
    'USING PERIODIC COMMIT LOAD CSV WITH HEADERS FROM "' + urlBase + '/rooms.csv" AS row MERGE (room:Room {roomId: row.roomId }) SET room.security = row.security, room.uri = row.uri, room.weight = toFloat(row.weight), room.lcOwner = row.lcOwner;',
