...
function getStat(tag, start, end) {
  var e = db.tagsynonyms.find({ synonym: tag }).map(function(f) { return f.name });
  var bases = [tag].concat(e);
  var synonyms = db.tagsynonyms.find({ name: { $in: bases } }).map(function(f) { return f.synonyms });
  if (synonyms) {
  ...
  var troupeIds = db.troupes.find({ tags: { $in: bases }}, { _id: 1 })
    .map(function(f) {
      return f._id;
    });
