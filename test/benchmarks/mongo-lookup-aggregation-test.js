    ...
    .then(function(rooms) {
      ...
      return persistence.SecurityDescriptor.find({
        ...
        .then(function(securityDescriptions) {
          var hashed = _.reduce(securityDescriptions, function(memo, sd) {
            memo[sd.troupeId] = sd;
            return memo;
          }, {});
...
makeBenchmark({
  ...
  before: function(done) {
    ...
    return fixtureLoader.createExpectedFixtures(expected)
      .then(function(f) {
        fixture = f;
      })
      .asCallback(done);
  ...
  tests: {
    'useQueryBig': function(done) {
      return useQuery(fixture.group1._id, fixture.user1._id)
        .asCallback(done);
    },
    'useAggregationBig': function(done) {
      return useAggregation(fixture.group1._id, fixture.user1._id)
        .asCallback(done);
    },
    'useDoubleQueryBig': function(done) {
      return doubleQuery(fixture.group1._id, fixture.user1._id)
        .asCallback(done);
    },

    'useQuerySmall': function(done) {
      return useQuery(fixture.group2._id, fixture.user1._id)
        .asCallback(done);
    },
    'useAggregationSmall': function(done) {
      return useAggregation(fixture.group2._id, fixture.user1._id)
        .asCallback(done);
    },
    'useDoubleQuerySmall': function(done) {
      return doubleQuery(fixture.group2._id, fixture.user1._id)
        .asCallback(done);
    },
